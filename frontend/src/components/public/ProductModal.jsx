...
export default function ProductModal({ product, onClose, lang = 'tr', isRefreshing = false }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative bg-cream w-full md:max-w-2xl md:rounded-3xl rounded-t-3xl max-h-[95vh] overflow-y-auto shadow-2xl z-10 animate-[slideUp_0.3s_ease]">
        ...
        {/* Loading Overlay */}
        {isRefreshing && (
          <div className="absolute inset-0 z-30 bg-white/40 backdrop-blur-[2px] flex items-center justify-center pointer-events-none transition-all duration-300">
             <div className="w-10 h-10 border-4 border-forest border-t-transparent rounded-full animate-spin shadow-lg"></div>
          </div>
        )}
        ...
        <div className={`transition-all duration-500 ${isRefreshing ? 'opacity-40 grayscale-[0.5]' : 'opacity-100'}`}>
          {/* Image Area */}
